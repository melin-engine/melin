...
**Melin** — a deterministic, replicated sequencer for latency-critical applications, built on the **LMAX architecture** (single-threaded business logic, event sourcing, mechanical sympathy). Rust (edition 2024). Provides the event-sourced processing pipeline, durable journaling, synchronous replication, transport (kernel TCP and DPDK), the application-agnostic server runtime, and a raft control plane (`crates/core/raft`, on openraft) for leader election and automatic failover — the one place tokio/serde are allowed, confined to its dedicated thread, never on the hot path. Applications (e.g. the Melin Exchange Core, maintained separately) plug in via the `melin-app` traits. `crates/core/client` (`melin-client`, published, Apache-2.0) is the client side of the wire protocol — framing, Ed25519 auth, the reply batch, a node's silence turned into an error; every example client and test harness talks to a node through it, and a new one should too rather than re-rolling the handshake. Three examples under `crates/examples/`: `echo` is a state-free application that doubles as the sequencer's latency floor (its client measures closed-loop round trips), `counter` is the reference (minimal) application with state, and `notary` exercises the ordering guarantee with a hash chain over client-submitted digests.
...
- **Dependencies are constrained by `deny.toml`** — licence on the allow list, source crates.io (git dependencies denied), no `*` version requirements. Copyleft is absent by design: needing it is a commercial decision, not a config edit. A new published workspace crate needs a `BUSL-1.1` exception, so that a third-party BSL dependency cannot pass as one of ours. The examples and `melin-client` are `Apache-2.0` (their manifests override the workspace licence and their `LICENSE` files carry the Apache text) so they can be copied into, or linked into, a customer's own code without the runtime's licence travelling along; a new example follows suit.
