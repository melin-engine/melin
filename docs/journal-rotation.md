...
Journal rotation prevents unbounded disk growth by archiving the active journal as a sealed segment and starting a fresh one. **Rotation runs while the node keeps serving** — no restart required.
...
The staged segment is pre-written with real zeros rather than merely space-allocated. This serves two purposes. First, rotation stays off the critical path. Second — and this matters for tail latency between rotations — appending into merely-allocated space forces a filesystem metadata commit inside `fdatasync` at unpredictable intervals on the filesystems that track unwritten regions (XFS, ext4); under sustained load this manifests as a periodic multi-millisecond stall of the whole pipeline (measured at ~2 ms every ~10 s on XFS). Appending into pre-written space never touches filesystem metadata, so the per-batch `fdatasync` stays on its fastest kernel path. The trade-off: every journal segment is written twice (zeros at staging time, data at append time), roughly doubling the journal device's sequential write volume — size the device's endurance budget accordingly. Disk-space headroom also changes shape: a fully pre-written next segment sits alongside the live one, so plan for one extra segment's worth of space beyond the archives you retain. Replicas pre-stage too (their rotation cost sits on the acknowledgement path, where it matters most), sizing the staged segment adaptively from the segment sizes the primary actually produces.

Pre-staging needs spare device *bandwidth*, not just spare endurance. Staging is deliberately paced to roughly a quarter of the journal device's throughput so it never crowds out the live segment's flushes — which means pre-writing a segment takes about four times as long as filling it with data. For pre-staging to stay ahead of rotation, keep the steady-state journal write rate below roughly a quarter of the device's sequential write throughput. Enterprise NVMe has ample headroom at the write rates a single sequencer generates, but a deployment that pushes the journal toward a large fraction of the device's throughput will find staging unable to keep up. That degrades visibly rather than silently: rotations revert to synchronous allocation and `melin_journal_rotations_total{path="sync_fallback"}` starts climbing. Note that raising `--max-journal-mib` does not help here: a larger segment takes proportionally longer to stage, so the ratio is independent of segment size. The remedies are a faster device or the `allocate` staging mode below.

On network-attached volumes (EBS and similar) sequential bandwidth is a metered resource that the staging pass and the live segment's own writes draw from together, and pre-writing roughly doubles what the journal takes from it. `--journal-staging-mode allocate` removes the pass: the next segment is reserved but not pre-written, staging costs no bandwidth, and the periodic metadata commit described above returns to the flush path. Which mode gives the better tail latency on a given volume is a property of that volume, so measure both under production load rather than assuming. The default remains `zero-fill`. The mode is fixed for the life of the process; changing it takes a restart.

The preparer runs on its own thread, pinned by the `journal-prep` entry of `--cores`. Leave it unpinned (`journal-prep=0`) or give it an auxiliary core — never a busy-spinning pipeline core, and never core 0. Left unpinned on a host with isolated cores, it runs on the non-isolated ones, so its staging bursts share them with the kernel and interrupt handling (see [CPU core pinning](pipeline-architecture.md#cpu-core-pinning)). It blocks in file I/O rather than polling, so it can share a core with the other yielding auxiliary threads and takes no wait suffix. Note that `journal-disk` *is* a pipeline thread: it writes and syncs the journal, and it wants a core to itself on the same CCD as `journal-seq`.

Two boundary cases keep the old behavior: the first segment after process start (including a live segment reopened by crash recovery) is allocated rather than pre-written, so the metadata-commit stall can appear until the first recurring rotation replaces it; and manual-only configurations (`--max-journal-mib 0` on a primary, rotating exclusively via the `ROTATE` admin command) skip pre-staging entirely — each manual rotation pays a synchronous allocate and its segment never gains the pre-written property. A third case is worth watching for: a pre-written segment that outgrows its staged region (an overshoot past the preparer's margin) loses the property for the rest of that segment, and the server logs a warning when that happens. Once is an overshoot; on every rotation it means the staged size is out of step with the segments actually being produced.
...
| `--journal-staging-mode <mode>` | `zero-fill` | How the next segment is staged. `zero-fill` pre-writes it (metadata-free flushes, one extra pass of device bandwidth per segment); `allocate` only reserves it (no staging bandwidth, periodic metadata commit on the flush path). See "Segment staging cost". |
