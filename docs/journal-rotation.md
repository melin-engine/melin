...
The preparer runs on its own thread, pinned via the optional tenth `--cores` entry (`journal-prep`). Leave it unpinned (`0`, the behavior when the entry is omitted) or give it an auxiliary core — never a busy-spinning pipeline core, and never core 0. It blocks in file I/O rather than polling, so it can share a core with the other yielding auxiliary threads and takes no wait suffix. Note that the eleventh entry (`journal-disk`) *is* a pipeline core: it pins the thread that writes and syncs the journal, and it wants a core to itself on the same CCD as `journal`.
