...
The preparer runs on its own thread, pinned by the `journal-prep` entry of `--cores`. Leave it unpinned (`journal-prep=0`, or leave it out of `--cores`) or give it an auxiliary core — never a busy-spinning pipeline core, and never core 0. On a host with isolated cores, give it a core: left unpinned there, it can end up sharing the journal thread's core (see [CPU core pinning](pipeline-architecture.md#cpu-core-pinning)). It blocks in file I/O rather than polling, so it can share a core with the other yielding auxiliary threads and takes no wait suffix. Note that `journal-disk` *is* a pipeline thread: it writes and syncs the journal, and it wants a core to itself on the same CCD as `journal`.
