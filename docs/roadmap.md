...
|---|---------|:---:|:---:|:---:|-----|
| 0 | Slow replica stalls primary pipeline | Critical | Medium | ★★★★★ | A connected replica that falls behind (slow fsync, network stall, bug) holds back the replication ring producer, which stalls the journal stage, which freezes the entire primary pipeline. The ring is SPMC — the producer gates on the **slowest** consumer, so one stuck replica takes down the cluster. **Options:** **(a)** Lossy ring — drop batches when full, replicas catch up from journal files (breaks lossless guarantee to connected replicas). **(b)** Timeout on backpressure — if the producer can't publish after N µs, disconnect the slow replica and mark its consumer idle for the drain loop (preserves lossless under normal conditions, degrades gracefully). **(c)** Independent rings — one ring per replica, journal stage skips full rings (isolates blast radius, more memory). Option (b) is the most practical. |
