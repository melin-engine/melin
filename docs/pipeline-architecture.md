...
1. Borrows a batch of events in place from the input ring — no copy out of the ring. This does not advance the progress cursor. The batch is capped at 4,096 events, or at whatever number of the application's events fills one hand-off slot, whichever is smaller.
...
| `MAX_JOURNAL_BATCH` | `4096` (ceiling; the effective cap is the lesser of this and what fills one hand-off slot) | `crates/core/transport-core/src/pipeline.rs` |
