...
- **The per-key duplicate-request gate: `Application::check_request_seq` and
  ...
  — live, on replay and in the shadow stage — and an application that
  refuses repeats does so there: it decodes the sequence into its own
  event, checks it keyed on `ApplyCtx::key_hash`, and builds its own
  rejection report. Source-breaking for every `Application`
...
- **The request sequence, from the wire, the journal and replication.** With
  ...
  the event; `InputSlot` and `JournalEntry` lose `request_seq`. An
  application that sequences requests puts the sequence in its own request
  body and event. Breaking on every surface it touched:
...
- **The shadow stage no longer applies a request the primary refused as a
  duplicate.** It asked the application for its verdict and then applied
  the event anyway, so a snapshot it wrote could hold a retried request
  twice, and a node restored from it would diverge from the primary. The
  shadow now hands every event to `apply`, as the matching stage and replay
  do, and the application's own check reaches the same verdict on all three.
...
[Unreleased]: https://github.com/melin-engine/melin/compare/v0.16.0...HEAD
