...
/// that cost against. A naive `last.elapsed() >= period` check in the
/// replica receiver + primary per-slot handler would reproduce as a
/// measurable throughput regression vs. the main-branch baseline.
///
...
pub(super) struct AmortizedTimer {
...
impl AmortizedTimer {
    ...
    /// clock read.
    #[inline]
    pub(super) fn tick(&mut self, period: std::time::Duration) -> Option<std::time::Duration> {
...
pub(super) fn update_dual_replication_cursor(
    this_acked: u64,
    other_acked: u64,
