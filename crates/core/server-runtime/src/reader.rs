    ...
    A::Report: Send + 'static,
{
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            // `core == 0` is the unpinned sentinel, which puts the thread
            // on the process's home CPU mask — see the `melin_app::affinity`
            // module docs.
            melin_app::affinity::pin_thread("uring-reader", core);
            reader_loop::<A, R>(
                rx,
                wakeup_fd,
                producer,
...
mod tests {
    ...
    /// A fenced node refuses with `Superseded`, whatever its replica count.
    #[test]
    fn a_fenced_reader_refuses_with_superseded() {
        let Fixture {
            mut conn,
            mut producer,
            mut consumer,
