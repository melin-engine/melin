    ...
    A::QueryResponse: Send + 'static,
{
    ...
    // the same recovery before releasing the flag, so the first tip the
    // raft driver trusts is already the recovered high-water mark.
    // Release-store so the driver's Acquire-load sees both.
        ...
        fn mid_stream_divergence_resyncs_in_process() {
            ...
            melin_transport_core::replication_wire::encode_input_batch(
                &[InputSlot::<EvtAdd> {
                    ...
                    key_hash: 0,
                    sequence: 1,
                    timestamp: melin_app::SequencerTime::from_ns(1),
            ...
            let send_events = |s: &mut TcpStream, range: std::ops::RangeInclusive<u64>| {
                let slots: Vec<InputSlot<EvtAdd>> = range
                    .map(|v| InputSlot {
                        ...
                        key_hash: 0,
                        sequence: v,
                        timestamp: melin_app::SequencerTime::from_ns(v),
