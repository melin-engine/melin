...
mod tests {
    ...
    fn save_load_round_trip() {
        ...
        for (label, chain) in [("populated", [0xCDu8; 32]), ("zero sentinel", [0u8; 32])] {
            ...
            save::<TestApp>(&app, 999, chain, 7, &path).unwrap();
    ...
    fn second_save_rotates_previous_to_prev() {
        ...
        save::<TestApp>(&app, 1, [0x11; 32], 0, &path).unwrap();
        assert!(path.exists());
        assert!(
            !prev_path.exists(),
        ...
        save::<TestApp>(&app, 2, [0x22; 32], 0, &path).unwrap();
        assert!(path.exists());
    ...
    fn checksum_mismatch_detected() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("snap");
        save::<TestApp>(&populated_app(), 0, [0u8; 32], 0, &path).unwrap();
    ...
    fn save_rejects_oversize_payload() {
        ...
        match save_with_limit::<TestApp>(&app, 0, [0u8; 32], 0, &path, /* max_size */ 16) {
