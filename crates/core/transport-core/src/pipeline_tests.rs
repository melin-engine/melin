...
/// stage: it neither advances its key's request-sequence mark nor drives
/// the scheduler clock, even when its slot carries a timestamp. Replay
/// never sees the query (the journal drops it), so either side effect
/// would leave the live engine in a state recovery cannot reproduce —
/// here, an `Add` under the query's sequence refused as a duplicate, or
/// a tick replay never fires.
#[test]
fn a_query_changes_no_matching_stage_state() {
    ...
    let slot = |event, request_seq, timestamp_ns| TestInput {
        connection_id: 1,
        key_hash: KEY,
        request_seq,
        sequence: 0,
    ...
    input_producer.publish(slot(JournalEvent::App(TestEvent::Add(1)), 10, 0));
    input_producer.publish(slot(JournalEvent::App(TestEvent::Query), 11, 1_000));
    input_producer.publish(slot(JournalEvent::App(TestEvent::Add(2)), 11, 0));
    ...
    assert!(
        ...
        "the query must not have claimed request sequence 11: {:?}",
        payloads[2]
    );
    ...
    assert_eq!(app.key_hwm.get(&KEY).copied(), Some(11));
}
