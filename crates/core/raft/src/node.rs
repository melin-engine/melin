...
impl ControlNode {
        ...
        for entry in entries {
            ...
            match entry.entry_type() {
                ...
                EntryType::EntryConfChange => {
                    ...
                    self.absorb_conf_change(index, applied)?;
                }
    ...
    ///
    /// On an `apply_conf_change` **rejection** we skip rather than
    /// halt. A committed entry was already durably agreed by a quorum,
    /// so the rejection means pre-proposal validation missed a case — a
    /// bug on our side, hence `error!`. Every node applies the same
    /// committed entry and would reject it identically, so returning
    /// the error here would brick the control plane on *every* node,
    /// deterministically and permanently. Staging the entry applied
    /// (without touching `ConfState`) lets all nodes converge past it.
    /// A storage error from the persist itself is a genuine local I/O
...
mod sim {
    ...
    fn committed_conf_change_that_fails_to_apply_does_not_halt() {
        ...
        // apply time, inside apply_committed. Before the hardening that
        // Err propagated out of drain_ready and halted the node (and,
        // cluster-wide, every node deterministically).
        {
        ...
        // panic here. The hardening absorbs the rejection.
        for _ in 0..20 {
            c.step_all();
        }

        let node = &c.nodes[&1];
        assert!(!node.halted(), "a rejected conf change must not halt");
