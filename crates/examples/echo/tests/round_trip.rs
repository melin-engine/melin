...
/// The paced client, run as a real process over kernel TCP: a short run
/// at a fixed rate, every reply back and checked, and the histogram on
/// disk under the name the Aeron harness would give it.
#[test]
fn the_bench_client_paces_a_run_and_writes_the_histogram() {
    let (tmp, server) = start_server();
    drop(connect_authenticated(server.addr, &trader_key()));
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_echo-bench"))
        .args([
            "--server",
            &server.addr.to_string(),
            "--key",
        ...
        .output()
        .expect("spawn echo-bench");
    let stdout = String::from_utf8(output.stdout).expect("utf-8 stdout");
    let stderr = String::from_utf8(output.stderr).expect("utf-8 stderr");
    assert_eq!(output.status.code(), Some(0), "{stdout}\n{stderr}");
    assert!(
        stdout.contains("sent 2000 received 2000 expected 2000"),
        "{stdout}\n{stderr}"
    );
    assert!(stdout.contains("MICROSECONDS"), "{stdout}");

    // Named as the harness names it, and one V2-compressed interval.
    let file = results.join(format!("echo_rate=2K_batch=1_length={MAX_PAYLOAD}.hdr"));
    assert!(file.is_file(), "{stdout}\n{stderr}");
    let text = std::fs::read_to_string(&file).expect("read histogram");
    assert!(text.lines().any(|l| l.contains(",HISTFAAA")), "{text}");

    // Warmup and measurement both went through the sequencer.
    server.stop();
    assert_eq!(journaled_echoes(tmp.path()).len(), 1_000 + 2_000);
}
