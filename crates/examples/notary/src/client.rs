...
use notary_server::{
    HEAD_LEN, LEAF_LEN, TAG_GET_HEAD, TAG_NOTARIZE, TAG_RESP_HEAD, TAG_RESP_RECEIPT,
    TAG_RESP_REJECTED, fold,
};
...
/// One link of the chain, as the client keeps it.
///
...
impl Receipt {
    ...
    fn from_frame(frame: &[u8], leaf: [u8; LEAF_LEN]) -> Result<Self, Error> {
        if frame.first() != Some(&TAG_RESP_RECEIPT)
            || frame.len() != 1 + 8 + 8 + HEAD_LEN + HEAD_LEN
        {
            return Err(format!("unexpected response to notarize: {}", hex(frame)).into());
        }
        Ok(Receipt {
            entry: u64::from_le_bytes(frame[1..9].try_into()?),
            timestamp_ns: u64::from_le_bytes(frame[9..17].try_into()?),
            leaf,
            prev: frame[17..49].try_into()?,
            head: frame[49..81].try_into()?,
        })
    }
    ...
    /// only mostly parses should not pass.
...
mod tests {
    ...
    #[test]
    fn receipt_text_tolerates_comments_and_whitespace() {
        let text = format!("# a comment\n\n  entry :  7 \n{}", &sample().to_text()[9..]);
        assert_eq!(Receipt::from_text(&text).unwrap(), sample());
    }

    #[test]
    fn receipt_text_is_strict() {
    ...
    #[test]
    fn receipt_frame_is_decoded_at_the_documented_offsets() {
        let receipt = sample();
