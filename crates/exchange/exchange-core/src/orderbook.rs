...
impl OrderBook {
    ...
    /// not on the hot path.
    ///
    /// Warm-safe: every structure self-guards on emptiness (see
    /// `SlabMap::prefault` / `BookSide::prefault`). On a warm book
    /// (replica promotion, snapshot/journal recovery), prefaulting a
    /// live index would wipe its entries, silently orphaning resting
    /// orders from their reservations — cancels stop finding them and
    /// the next snapshot drops their reservations, so a clone fills
    /// against `ReservationSlot::DUMMY` and panics.
    pub fn prefault(&mut self) {
        self.order_index.prefault((
            Side::Buy,
            Price(std::num::NonZeroU64::new(1).expect("non-zero literal")),
            ReservationSlot::DUMMY,
            INVALID_NODE,
        ));
        self.stop_index.prefault((
            Side::Buy,
            Price(std::num::NonZeroU64::new(1).expect("non-zero literal")),
            INVALID_NODE,
        ));
