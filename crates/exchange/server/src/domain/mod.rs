...
//! `melin_engine::exchange::Exchange`, the wire-`Request` decoder, the
//! `OutputPayload` response encoders (kernel and DPDK), and the
//! market-data firehose publisher. When `core` becomes fully generic
//! these stay behind in this crate (or move to a sibling
//! `trading-server` crate) as the trading adapter.
