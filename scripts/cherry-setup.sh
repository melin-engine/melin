...
echo "  3. cd $REPO_DIR"
echo ""
echo "  4. TCP benchmark:"
echo "     sudo ./scripts/bench-isolate.sh --features no-persist -- 1000000"
echo ""
echo "  5. UDP benchmark (server):"
echo "     sudo RUST_LOG=info cargo run --release -p trading-server --features no-persist,pipeline-stats -- --udp --bind <vlan-ip>:9876 --journal /mnt/journal/bench.journal --accounts 1000 --instruments 100"
echo "     # Then from the bench machine:"
echo "     cargo run --release -p trading-bench -- 1000000 --udp --addr <server-vlan-ip>:9876"
echo ""
