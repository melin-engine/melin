...
#   DPDK_EAL_ARGS     EAL arguments (default: "--no-pci --vdev=net_tap0")
#                     For real NIC: "" (empty, use default PCI scan)
#   DPDK_PORT         DPDK port ID (default: 0)
#   DPDK_CORE         CPU core for DPDK poll thread (default: 7)
...
# ---------------------------------------------------------------------------
echo "=== Setting up hugepages on server ==="
ssh $SSH_OPTS "$SERVER" "\
