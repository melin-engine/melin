...
cleanup() {
    for host in "$SERVER" ${REPLICA:+"$REPLICA"} ${REPLICA2:+"$REPLICA2"}; do
        ssh $SSH_OPTS "$host" "pkill -INT -x melin-server 2>/dev/null; \
                               pkill -INT -x melin-server.rumcast 2>/dev/null; \
                               pkill -INT -x melin-server.dpdk 2>/dev/null; true" 2>/dev/null || true
    done
...
stop_servers() {
    for host in "$@"; do
        ...
        ssh $SSH_OPTS "$host" "pkill -INT -x melin-server 2>/dev/null; \
                               pkill -INT -x melin-server.rumcast 2>/dev/null; \
                               pkill -INT -x melin-server.dpdk 2>/dev/null; true"
    done
    sleep 2
}
...
transport_start_udp() {
    ...
    ssh $SSH_OPTS "$SERVER" "pkill -x melin-server.rumcast 2>/dev/null; pkill -x melin-server 2>/dev/null; true"
    sleep 1
...
transport_start_udp_repl() {
    ...
    ssh $SSH_OPTS "$REPLICA" "pkill -x melin-server.rumcast 2>/dev/null; pkill -x melin-server 2>/dev/null; true"
    sleep 1
...
transport_start_udp_dual_repl() {
    ...
    ssh $SSH_OPTS "$REPLICA2" "pkill -x melin-server.rumcast 2>/dev/null; pkill -x melin-server 2>/dev/null; true"
    sleep 1
